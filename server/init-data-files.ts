...
export async function initializeDataFiles(): Promise<void> {
    try {
        ...
        // Handle sectors_aggregated_metrics.json - try minimal version first
        const sectorsAggregatedPath = path.join(destinationDataPath, 'sectors_aggregated_metrics.json');
        const sectorsMinimalPath = path.join(sourceDataPath, 'sectors_aggregated_metrics.minimal.json');
        
        if (!fs.existsSync(sectorsAggregatedPath)) {
            // Try to copy minimal version if it exists
            if (fs.existsSync(sectorsMinimalPath)) {
                const destMinimalPath = path.join(destinationDataPath, 'sectors_aggregated_metrics.json');
                try {
                    fs.copyFileSync(sectorsMinimalPath, destMinimalPath);
                    console.log(`[DataInit] Copied sectors_aggregated_metrics.minimal.json as sectors_aggregated_metrics.json`);
                } catch (error) {
                    console.error(`[DataInit] Error copying minimal sectors file:`, error);
                }
            } else {
