...
export function MetricsCards({ keywords }: MetricsCardsProps) {
    ...
    const calculateWeightedAverage = (getValue: (k: Keyword) => number) => {
        ...
        const totalWeight = keywords.reduce((sum, k) => {
            const similarityScore = k.similarityScore || 0;
            const matchPct = typeof similarityScore === 'string'
                ? parseFloat(similarityScore.replace('%', '')) / 100
                : similarityScore; // Already a number between 0-1
